        ...
        protected void CreatePatchVertices(Vector3[] patch, int tessellation, bool isMirrored)
        {
            ...
            for (var i = 0; i <= tessellation; i++)
            {
                ...
                for (var j = 0; j <= tessellation; j++)
                {
                    ...
                    // Perform a vertical interpolation between the results of the previous horizontal interpolations, to compute the position.
                    var position = Bezier(p1, p2, p3, p4, tj);

                    // Perform another four bezier interpolations between the control points, but this time vertically rather than horizontally.
